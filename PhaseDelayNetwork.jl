...
function PhaseDelayNetwork(du,u,h,p,t)
    P1, W, τ = p
    @unpack N, ω, ϵ, v, σ, τ0 = P1
    for i = 1:N
        du[i] = ω     #originally θ̇ᵢ = ω
        for j = 1:N
            hist = h(p, t - τ[i,j])[j]
...
gif(anim, "PhaseDelayNetwork2.gif", fps=10)


#Plot R vs Time
plt2 = plot(t, R, lw=2, xlabel="Time", ylabel="R(t)", title="Order Parameter vs Time", legend=false)
display(plt2)
